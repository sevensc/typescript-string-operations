...
(function (sf) {
    var String = (function () {
        ...
        String.Join = function (combineString) {
            var args = [];
            for (var _i = 1; _i < arguments.length; _i++) {
                args[_i - 1] = arguments[_i];
            }
            try {
                if ($.isArray(args[0]) || args[0] === typeof sf.List) {
                    var tempString = String.Empty;
                    var count = 0;
                    for (var i = 0; i < args[0].length; i++) {
                        var current = args[0][i];
                        if (i < args[0].length - 1)
                            tempString += current + combineString;
                        else
                            tempString += current;
                    }
                    return tempString;
                    ...
                    $(args[0]).each(function () {
                        if (count < args[0].length - 1)
                            tempString += $(this).text() + combineString;
                        else
                ...
                return String.join(combineString, args);
            }
            catch (e) {
                console.log(e);
                return String.Empty;
        ...
        String.Format = function (value) {
            ...
            try {
                return value.replace(/{(\d+(:\w*)?)}/g, function (match, i) {
                    var s = match.split(':');
                    if (s.length > 1) {
                        i = i[0];
                        match = s[1].replace('}', ''); //U
        ...
        String.join = function (combineString, args) {
